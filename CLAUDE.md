...
- **Config**: `server.properties` — port 25565, `online-mode=true`, `level-name=world-settlers-board`, gamemode creative, nether/end disabled, RCON disabled. `spigot.yml` has `restart-on-crash: true` → `./start.sh` (script doesn't exist here — legacy artifact).
...
- **Worlds**:
  - `world-settlers-board` — the ACTIVE game world; gitignored; recreated each game by rsync from template.
  - `world-empty-settlers-board` (19MB) — blank-slate template used for resets.
  - `world-settlers-templates` (39MB) — contains the structure templates (tiles, settlements, etc.) that WorldEdit loads during board build. Keep its datapack in sync too.
  - `world-empty-settlers-board-beaches` (43MB) — variant template, currently unreferenced by scripts.
...
- `rsync --delete` world resets are destructive by design — only ever target `world-settlers-board`.
